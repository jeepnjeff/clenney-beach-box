...
/*
    ...
    V2.1 - 29JAN2019 - REF:1 (XSS Title vulnberability)
                        -Added filters to title text so it is escapped correctly preventing XSS attacks
*/
    ...
    if ( $post->have_posts() ) {
        while ( $post->have_posts() ) {
            ...
            if ($theContent==""){
                ...
                $output .= '<div id="parallax_'.$postid.'_posttitle" class="adamrob_parallax_posttitle" style="'.$hStyle.'">' . $TheTitle  . '</div>';
                $output .= '</td></tr></table>';
                ...
                $output = $output . '<div style="'.$hStyle.'">' . $TheTitle  . '</div>';
                $output = $output . '</div>';
